...
**目的**：在 `docs/designs/market-mind-identity-access-domain.md` 的域边界与原则之上，描述 **market-backend** 侧如何支撑：PC 端、**微信内置浏览器内网页授权**，以及与现有 **Google OAuth** 并存时的数据模型、路由与安全约定。  
...
- [ ] 实现 `upsertUserFromWechat` 与 `users.email` 占位策略（或 schema 迁移）
- [ ] 增加微信路由与 state 校验，复用 `issueTokenPair` 与 exchange code 回跳
- [ ] 为 exchange-code 提供多 IdP 兼容路径与回归测试
