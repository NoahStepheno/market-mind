...
┌─────────────────────────────────────────────────────────────┐
│ 告警域                                                           │
│  alarms、评估、notifications …（见 market-mind-alarm-domain.md） │
└─────────────────────────────────────────────────────────────┘
```
...
- **Session（会话）**：用户与 MarketMind 的一条连续对话容器；V1 可「单会话多轮」或「每开一个创建流新建会话」，产品决定；工程上需 `user_id` + `session_id`。
...
`conditionGroup`、`cooldown`、`notifyLabel`、`notifyTier` 等字段含义与类型以 PRD §4.1 与 `market-mind-alarm-domain.md` §4 为准；聊天域**生成同形 JSON** 作为创建请求体即可。
...
1. **任务说明**（短系统提示）：MarketMind V1 只做告警草稿、字段受 PRD 约束等。
...
- 产品主流程与数据结构：`docs/prds/market-mind-prd.md`
- 告警表结构、软删除、`POST /alarms`、队列载荷：`docs/designs/market-mind-alarm-domain.md`
- 三域划分、`notifications` 归属：`docs/designs/market-mind-domain-model.md`
- 身份与鉴权：`docs/designs/market-mind-identity-access-domain.md`
